...
class CustomSparseEncoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 encoder_channels=((16, ), (32, 32, 32), (64, 64, 64), (64, 64,
                                                                        64)),
                 encoder_strides=((1, ), (2, 1, 1), (2, 1, 1), (2, 1, 1)),
                 encoder_paddings=((1, ), (1, 1, 1), (1, 1, 1), ((0, 1, 1), 1,
                                                                 1)),
                 block_type='conv_module',
                 with_final_bnrelu=True,
                 output_volume_feat=False):
        super().__init__()
        assert block_type in ['conv_module', 'basicblock']
        self.sparse_shape = sparse_shape
        self.in_channels = in_channels
        ...
        self.encoder_paddings = encoder_paddings
        self.with_final_bnrelu = with_final_bnrelu
        self.output_volume_feat = output_volume_feat
        self.stage_num = len(self.encoder_channels)
        self.fp16_enabled = False
        # Spconv init all weight on its own
        ...
        if self.with_final_bnrelu:
            ...
            # only used for LIGA SparseEncoder
            # use different kernel_size and stride
            self.conv_out = make_sparse_convmodule(
                encoder_out_channels,
                self.output_channels,
                kernel_size=1,
    ...
    def forward(self, voxel_features, coors, batch_size):
        ...
        # [200, 176, 5] -> [200, 176, 2]
        out = self.conv_out(encode_features[-1])
        volume_features = out.dense()

        N, C, D, H, W = volume_features.shape
        spatial_features = volume_features.view(N, C * D, H, W)

        outputs = spatial_features

        if self.output_volume_feat:
            outputs = (spatial_features, volume_features)
    ...
    def make_encoder_layers(self,
        ...
        for i, blocks in enumerate(self.encoder_channels):
            blocks_list = []
            for j, out_channels in enumerate(tuple(blocks)):
                stride = tuple(self.encoder_strides[i])[j]
                padding = tuple(self.encoder_paddings[i])[j]
                # each stage started with a spconv layer
                # except the first stage
                if i != 0 and j == 0 and block_type == 'conv_module':
